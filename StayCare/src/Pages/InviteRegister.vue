...
const handleRegister = async () => {
  if (!name.value || !password.value) {
    error.value = t('inviteRegister.errorNamePasswordRequired')
    return
  }
  if (password.value.length < 6) {
    error.value = t('inviteRegister.errorPasswordMin')
    return
  }
  if (password.value !== confirmPassword.value) {
    error.value = t('inviteRegister.errorPasswordMismatch')
    return
  }
  ...
  try {
    ...
    error.value = err?.message || t('inviteRegister.errorRegistrationFailed')
  } finally {
...
<template>
  <div class="template">
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center bg-white px-8 py-12 rounded-lg shadow-lg max-w-md w-full" style="position: relative; z-index: 1;">
      ...
      <p class="mt-4 text-gray-500">{{ $t('inviteRegister.validatingInvitation') }}</p>
    </div>
    ...
    <div v-else-if="invalid" class="flex flex-col items-center justify-center bg-white px-8 py-12 rounded-lg shadow-lg max-w-md w-full" style="position: relative; z-index: 1;">
      <h1 class="text-2xl font-bold text-red-500">{{ $t('inviteRegister.invalidTitle') }}</h1>
      <p class="mt-4 text-gray-600 text-center">{{ $t('inviteRegister.invalidDescription') }}</p>
      <AppButton @click="router.push('/logorcreate')" class="mt-6 w-full">
        {{ $t('inviteRegister.goToLogin') }}
      </AppButton>
    </div>
    ...
    <div v-else-if="expired" class="flex flex-col items-center justify-center bg-white px-8 py-12 rounded-lg shadow-lg max-w-md w-full" style="position: relative; z-index: 1;">
      <h1 class="text-2xl font-bold text-amber-500">{{ $t('inviteRegister.expiredTitle') }}</h1>
      <p class="mt-4 text-gray-600 text-center">{{ $t('inviteRegister.expiredDescription') }}</p>
      <AppButton @click="router.push('/logorcreate')" class="mt-6 w-full">
        {{ $t('inviteRegister.goToLogin') }}
      </AppButton>
    </div>
    ...
    <div v-else class="flex flex-col items-center justify-center bg-white px-8 py-10 rounded-lg shadow-lg max-w-md w-full" style="position: relative; z-index: 1;">
      <h1 class="text-3xl font-bold text-center text-brand-700">{{ $t('inviteRegister.joinTitle') }}</h1>

      <div class="mt-4 bg-brand-150 rounded-lg px-4 py-3 w-full text-center">
        <p class="text-sm text-gray-600">{{ $t('inviteRegister.joiningAs') }}</p>
      ...
      <input
        type="text"
        :placeholder="$t('inviteRegister.fullName')"
        v-model="name"
      ...
      <input
        type="tel"
        :placeholder="$t('inviteRegister.phone')"
        v-model="phone"
      ...
      <input
        type="password"
        :placeholder="$t('inviteRegister.password')"
        v-model="password"
        class="mt-3 border-2 border-gray-300 bg-gray-50 rounded-lg px-4 py-2 w-full focus:outline-none focus:border-brand-700 focus:ring-2 focus:ring-brand-700/40"
      />
      ...
      <input
        type="password"
        :placeholder="$t('inviteRegister.confirmPassword')"
      ...
      <AppButton
        ...
        class="mt-6 w-full"
      >
        {{ submitting ? $t('inviteRegister.creatingAccount') : $t('inviteRegister.createAccount') }}
      </AppButton>
    </div>
  </div>
</template>
