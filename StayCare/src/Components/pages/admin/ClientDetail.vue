    ...
    <template v-if="client">
      ...
      <div class="bg-white rounded-xl shadow-sm p-5 space-y-3">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          <div><span class="text-gray-500">{{ $t('clientDetail.company') }}</span><p class="font-medium text-gray-800">{{ client.company_name ?? client.name ?? '—' }}</p></div>
          ...
          <div><span class="text-gray-500">{{ $t('clientDetail.paymentTerms') }}</span><p class="font-medium text-gray-800">{{ client.credits_terms_days ?? '0' }} {{ $t('clientDetail.days') }}</p></div>
