<template>
  <div class="space-y-6 max-w-3xl">
    ...
    <div class="flex items-center gap-3">
      <button @click="navStore.goBack('invoices')" class="text-gray-400 hover:text-gray-600">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
      </button>
      <h2 class="text-lg font-semibold text-gray-800">{{ invoice?.id }}</h2>
    ...
    <div v-if="invoice" class="space-y-6">
      ...
      <div class="bg-white rounded-xl shadow-sm p-5">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
          <div><span class="text-gray-400">Client</span><p class="font-medium text-gray-800">{{ invoice.client }}</p></div>
          <div><span class="text-gray-400">Order</span><p class="font-medium text-gray-800">{{ invoice.orderId }}</p></div>
          <div><span class="text-gray-400">Issue Date</span><p class="font-medium text-gray-800">{{ invoice.issueDate }}</p></div>
          <div><span class="text-gray-400">Due Date</span><p class="font-medium text-gray-800">{{ invoice.dueDate }}</p></div>
          <div v-if="invoice.paymentMethod"><span class="text-gray-400">Payment Method</span><p class="font-medium text-gray-800">{{ invoice.paymentMethod }}</p></div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100">
          <h3 class="text-sm font-semibold text-gray-700 uppercase tracking-wide">Line Items</h3>
        </div>
      ...
      <div class="bg-white rounded-xl shadow-sm p-5">
        <div class="space-y-2 text-sm max-w-xs ml-auto">
          <div class="flex justify-between"><span class="text-gray-500">Subtotal</span><span class="font-medium">&euro;{{ invoice.subtotal.toFixed(2) }}</span></div>
          <div v-if="invoice.expressCharge > 0" class="flex justify-between"><span class="text-gray-500">Express Surcharge</span><span class="font-medium">&euro;{{ invoice.expressCharge.toFixed(2) }}</span></div>
          <div class="flex justify-between"><span class="text-gray-500">VAT (18%)</span><span class="font-medium">&euro;{{ invoice.vat.toFixed(2) }}</span></div>
    ...
    <div v-else class="bg-white rounded-xl shadow-sm p-10 text-center">
      <p class="text-gray-400">Invoice not found.</p>
    </div>
  </div>
</template>
