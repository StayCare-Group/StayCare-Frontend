  ...
  <div class="space-y-6 max-w-3xl">
    ...
    <form @submit.prevent="submitOrder" class="space-y-6">
      <div class="bg-white rounded-xl shadow-sm p-5 space-y-4">
        <h3 v-if="isAdmin" class="text-sm font-semibold text-gray-700 uppercase tracking-wide">{{ t('admin.selectClient') }}</h3>
        <h3 v-else class="text-sm font-semibold text-gray-700 uppercase tracking-wide">{{ t('admin.pickupDetails') }}</h3>
        ...
        <div>
          ...
          <p v-if="isAdmin && form.clientId && !loadingClient && !properties.length" class="text-xs text-amber-500 mt-1">
      ...
      <div class="bg-white rounded-xl shadow-sm p-5 space-y-4">
        ...
        <div class="divide-y divide-gray-100">
          <div v-for="item in laundryItems" :key="item.code" class="flex items-center justify-between py-3 gap-4">
            ...
            <input
              v-model.number="itemQtys[item.code]"
              type="number"
              min="0"
